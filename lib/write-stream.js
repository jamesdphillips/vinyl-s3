...
module.exports = function createWriteStream(root, options) {
    ...
    // Do not include Content-Encoding header unless the file is actually
    // encoded. Otherwise an empty string in the Content-Encoding header causes
    // Cloudfront's compressed files feature to ignore the file. 💩
    if (!_.isEmpty(contentEncoding)) {
      computedOptions.ContentEncoding = contentEncoding.join(',');
    }

    var fileOptions = _.assign(
      { },
      awsOptions,
      computedOptions,
      file.awsOptions
